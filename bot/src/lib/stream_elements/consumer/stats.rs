...
impl Stats {
    ...
    pub fn with_settings(self, settings: StatsSettings) -> Self { Self { settings, ..self } }
    ...
    /// Retrieves the stats of the given channel. Requires the permission to
    /// view channel stats.
...
impl UserData for Stats {
    fn add_methods<'lua, M: UserDataMethods<'lua, Self>>(methods: &mut M) {
        ...
        methods.add_async_method("stats_for_channel", |lua, instance, channel_id: String| async move {
        ...
        methods.add_method("settings", |lua, instance, _: ()| {
            ...
            table.set("date", format!("{}", instance.settings.date.format("%Y-%m-%d")))?;
