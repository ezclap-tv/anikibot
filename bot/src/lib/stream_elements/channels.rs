...
impl<'a> Channels<'a> {
    ...
    #[inline(always)]
    pub async fn me(&self) -> APIResult<Response> { self.channel("me").await }

    /// Retrieves the channel id of the API user.
    #[inline(always)]
    pub async fn my_id(&self) -> Result<String, BackendError> { self.channel_id("me").await }
    ...
    pub async fn channel(&self, name_or_id: &str) -> APIResult<Response> {
        self.api.get(&format!("channels/{}/", name_or_id)).send().await
    }
        ...
        self.channel(name)
            ...
            .and_then(|v| {
                v.get("_id")
                    ...
                    .ok_or_else(|| BackendError::from(format!("Failed to fetch the channel id for `{}`", name)))
            })
    }
}
