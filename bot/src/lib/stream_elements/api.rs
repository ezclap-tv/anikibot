...
use super::{
    communication::spawn_api_thread, config::StreamElementsConfig, consumer::ConsumerStreamElementsAPI,
    song_requests::SongRequests, stats::Stats,
};
...
impl StreamElementsAPIGuard {
    ...
    /// Checks that the channel_id is present in the config. If not, requests it
    /// from the StreamElements API via `GET: channels/me/`.
...
impl StreamElementsAPI {
    ...
    /// To obtain a usable API object, the user must call [`start()`] and verify
    ...
    pub fn with_config(config: StreamElementsConfig) -> StreamElementsAPIGuard {
        ...
        StreamElementsAPIGuard {
            api: Self {
                config,
                client: Client::builder().default_headers(headers).build().unwrap(), /* Only fails if the TLS backend
                                                                                      * or config is invalid */
            },
        }
    }

    /// Makes a deep copy of the API. Useful for sending API objects to
    /// different threads.
    #[inline]
    ...
    /// Formats `BASE_API_URL` with the given `channel_id`, `method`, and
    /// `endpoint` to obtain an API method URL.
    ///
    /// ```
    ...
    /// Formats `BASE_API_URL` with the given `endpoint` to obtain an API method
    /// URL.
    ///
    /// ```
    /// # use backend::stream_elements::api::StreamElementsAPI;
    ...
    /// ```
    #[inline]
    pub fn get_endpoint_url(endpoint: &str) -> String { format!("{}/{}", BASE_API_URL, endpoint) }

    /// Returns the configured channel id.
    #[inline(always)]
    pub fn channel_id(&self) -> &str { &self.config.channel_id }
    ...
    /// Builds a request for the given API method.
    #[inline]
    pub(crate) fn get_method_for_channel_id(&self, channel_id: &str, method: &str, endpoint: &str) -> RequestBuilder {
    ...
    pub(crate) fn post_method_for_channel_id(&self, channel_id: &str, method: &str, endpoint: &str) -> RequestBuilder {
        let url = StreamElementsAPI::get_method_endpoint_url(channel_id, method, endpoint);
        log::debug!("POST: {}", url);
